...
#include "batrobot_msgs/action/batrobot_task.hpp"
...
public:
    ...
    : Node("task_server", options)
  {
    ...
    action_server_ = rclcpp_action::create_server<batrobot_msgs::action::BatrobotTask>(
...
private:
  rclcpp_action::Server<batrobot_msgs::action::BatrobotTask>::SharedPtr action_server_;
  ...
  rclcpp_action::GoalResponse goalCallback(
      ...
      std::shared_ptr<const batrobot_msgs::action::BatrobotTask::Goal> goal)
  {
  ...
  rclcpp_action::CancelResponse cancelCallback(
      const std::shared_ptr<rclcpp_action::ServerGoalHandle<batrobot_msgs::action::BatrobotTask>> goal_handle)
  {
    (void)goal_handle;
  ...
  void execute(const std::shared_ptr<rclcpp_action::ServerGoalHandle<batrobot_msgs::action::BatrobotTask>> goal_handle)
  {
    RCLCPP_INFO(get_logger(), "Executing goal");
    auto result = std::make_shared<batrobot_msgs::action::BatrobotTask::Result>();

    // MoveIt 2 Interface
    if(!arm_move_group_){
      arm_move_group_ = std::make_shared<moveit::planning_interface::MoveGroupInterface>(shared_from_this(), "arm_planning_group");
    }
    if(!gripper_move_group_){
      gripper_move_group_ = std::make_shared<moveit::planning_interface::MoveGroupInterface>(shared_from_this(), "gripper_planning_group");
    }

    if (goal_handle->get_goal()->task_number == 0)
    {
      arm_joint_goal_ = {0.0, 0.0, 0.0, -1.57};
      gripper_joint_goal_ = {0.0, 0.0};
    }
    else if (goal_handle->get_goal()->task_number == 1)
    {
      arm_joint_goal_ = {1.57, 0.0, 0.0, 1.57};
      gripper_joint_goal_ = {0.0, 0.0};
    }
...
}  // namespace batrobot_remote

RCLCPP_COMPONENTS_REGISTER_NODE(batrobot_alexa::TaskServer)
